...
[ApiController]
[ApiVersion("1.0")]
[Route("api/v{version:apiVersion}/meuprogresso")]
[Authorize]
public class MeuProgressoController : ControllerBase
{
    private readonly AppDbContext _context;
    private readonly IDataCacheService _cache;

    public MeuProgressoController(AppDbContext context, IDataCacheService cache)
    {
        _context = context;
        _cache = cache;
    }
    ...
    [HttpGet]
    public async Task<ActionResult<PagedResult<MeuProgresso>>> GetAll([FromQuery] int pageNumber = 1, [FromQuery] int pageSize = 10)
    {
        if (pageNumber <= 0) pageNumber = 1;
        if (pageSize <= 0) pageSize = 10;

        var query = _context.MeusProgresso.AsNoTracking();
        ...
        var result = new PagedResult<MeuProgresso>(items, pageNumber, pageSize, totalCount, totalPages, links);
        return Ok(result);
    }

    [HttpGet("{id:int}")]
    public async Task<ActionResult<MeuProgresso>> GetById(int id)
    {
        var item = await _context.MeusProgresso.FindAsync(id);
        if (item == null)
        {
            return NotFound();
        }
    ...
    [HttpPost]
    public async Task<ActionResult<MeuProgresso>> Create([FromBody] MeuProgresso item)
    {
        _context.MeusProgresso.Add(item);
    ...
    [HttpPut("{id:int}")]
    public async Task<IActionResult> Update(int id, [FromBody] MeuProgresso item)
    {
        if (id != item.Id)
        {
        ...
        var existing = await _context.MeusProgresso.FindAsync(id);
        if (existing == null)
        {
            return NotFound();
        }
        ...
        existing.CursoId = item.CursoId;
        existing.Status = item.Status;
        existing.DtConclusao = item.DtConclusao;
    ...
    public async Task<IActionResult> Delete(int id)
    {
        ...
        _context.MeusProgresso.Remove(existing);
        await _context.SaveChangesAsync();
